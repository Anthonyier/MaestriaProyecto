...
{
    public partial class FormImgPagasChequesAceite : System.Web.UI.Page
    {
            ...
            if (IdEstado == 0)
            {
                if (this.FileUpPagado.HasFile)
                {
                    ...
                    if (DocPagadas.Imagen != null)
                    {
                        ...
                        int modPago = 101;
                        string modalidad = "CHEQUE";
                        int IdCuenta = Convert.ToInt32(DdlCuentaBancaria.Text);
                        if (IdFactura == -1)
                        {
                            int IdAdelantoRec = NegAdelantoDePago.NEncontrarAdelPagoReciboAceite(DocPagadas.IdConciliacionAceite);
                            int IdDescuentoRec = NegAdelanto.NEncontrarDescuentoReciboAceite(DocPagadas.IdConciliacionAceite);
                            Ver = NegImgPagadas.MaestroProveedorReciboAceite(DateTime.Parse(TextFecha.Text), IdPer, IdUsuario, MontoPorPagar, DocPagadas, idTipoPago, IdPasivo, IdAdelantoRec, IdDescuentoRec, modPago, modalidad,IdCuenta);
                        }
                        else
                        {
                            ...
                            Ver = NegImgPagadas.MaestroProveedorAceite(DateTime.Parse(TextFecha.Text), IdPer, IdUsuario, MontoPorPagar, DocPagadas, idTipoPago, IdPasivo, IdAdelanto, IdDescuento, IdFactura, modPago, modalidad,IdCuenta);
                        }
                ...
                Response.Redirect("FormListaSolPagoAceCheque.aspx");
            }
        }
