...
{
    public partial class FormSolicitudPagosAceiteNuevoFormato : System.Web.UI.Page
    {
        ...
        protected void buttonCheck_Click(object sender, EventArgs e)
        {
            ...
            if (Lista.Count() > 0)
            {
                ...
                {
                    int Id_Pmm = NegConciliacionPagarAceite.InsertarPagoMasivoLiqui();
                    for (int i = 0; i < Lista.Count(); i++)
                    {
                        int Id = Lista[i];
                        NegConciliacionPagarAceite.ActualizarConciliacionPorPagarIdPmm(Id_Pmm, Id);
                    }
        ...
        protected static bool VerificarYaPagado(List<int> Lista)
        {
            ...
            {
                int Id = Lista[i];
                int Verif = NegConciliacionPagarAceite.VerificarYaPagadoLiquidacion(Id);
                if (Verif != 0)
                {
                    YaPag = true;
                    return YaPag;
        ...
        protected void CrearTexto(int IdDetalle)
        {
            ...
            int IdPagoMasiv = NegConciliacionPagarAceite.EncontrarIdPagoMasiv(IdDetalle);
            if (IdPagoMasiv == 0)
            {
