...
{
    public partial class FormListaRutasCambiar : System.Web.UI.Page
    {
        ...
        protected void DtgListaRutaCambiar_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            EntUsuario usuario = (EntUsuario)Session["Usuario"];
            EntPermisoRutas Per = NegPermisoRutas.BuscarPermiso(usuario.Id_Usuario);
            if (e.CommandName == "Modificar")
            {
                if (Per.ModificarRutas == 1)
                {
                    string DetalleRecpId = e.CommandArgument.ToString();
                    Response.Redirect("FormCambioRutaViaje.aspx?Id=" + DetalleRecpId);
                }
            }
        }
    }
