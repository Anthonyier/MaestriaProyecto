...
{
    public partial class FormPagoSinFactura : System.Web.UI.Page
    {
            ...
            if (IdEstado == 0)
            {
                if (this.FileUpPagado.HasFile)
                {
                    ...
                    if (DocPagadas.Imagen != null)
                    {
                        ...
                        bool Ver = NegImgPagadas.MaestroProveedor(DateTime.Parse(TextFecha.Text), IdPer, IdUsuario, MontoPorPagar, DocPagadas, idTipoPago, IdPasivo,IdAdelanto,IdDescuento,IdFactura,modPago,modalidad,IdCuenta);
                        if (Ver == true)
                        {
                            NegConciliacionPorPagar.Pagado(int.Parse(Request.QueryString["Id"]));
                            NegConciliacionPorPagar.PagadoSinFactura(int.Parse(Request.QueryString["Id"]));
