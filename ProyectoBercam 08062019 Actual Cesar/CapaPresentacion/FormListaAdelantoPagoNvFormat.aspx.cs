...
{
    public partial class FormListaAdelantoPagoNvFormat : System.Web.UI.Page
    {
        ...
        protected void buttonCheck_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                ...
                {
                    Label LblNroCuenta = (GridView1.Rows[i].Cells[0].FindControl("Label6") as Label);
            ...
            if (Lista.Count() > 0)
            {
                ...
                {
                    int Id_Pmm = NegAdelantoDePago.InsertarPagoMasivoAdel();
                    for (int i = 0; i < Lista.Count(); i++)
                    {
                        int Id = Lista[i];
                        NegAdelantoDePago.ActualizarIdAdelantoDePago(Id_Pmm, Id);
                    }
        ...
        protected static bool VerificarYaPagado(List<int> Lista)
        {
            ...
            {
                int Id = Lista[i];
                int Verif = NegAdelantoDePago.VerificarYaPagadoAdel(Id);
                if (Verif != 0)
                {
                    YaPag = true;
                    return YaPag;
        ...
        protected void DtgListaGridViewAdelPmm_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            EntUsuario us = (EntUsuario)Session["Usuario"];
            EntUsuario usuario = (EntUsuario)Session["Usuario"];
            EntPermisosAdelantosDePago  Adelanto = NegPermisosAdelantosDePago.BuscarPermiso(usuario.Id_Usuario);
            if (e.CommandName == "ModoTexto")
            {
                if (Adelanto.PagarAdelantoDePago == 1)
                {
        ...
        protected void CrearTexto(int IdDetalle)
        {
            ...
            int IdPagoMasiv = NegAdelantoDePago.EncontrarIdPagoMasiv(IdDetalle);
            if (IdPagoMasiv == 0)
            {
            ...
            {
                Label LblIdPago = (Rows.FindControl("Label7") as Label);
                int Idpmm = 0;
                ...
                {
                    Label LblNroCuenta = (Rows.FindControl("Label6") as Label);
                    long NroCuenta = Convert.ToInt64(LblNroCuenta.Text);
                    string AbrevBanco = NegAsignacionRuta.EncontrarAbrevBanco(NroCuenta);
                    string Tipo = "";
                    Label LblMontoAdelanto = (Rows.FindControl("Label2") as Label);
                    double MontoAdelanto = Convert.ToDouble(LblMontoAdelanto.Text);
                    ...
                    Label LblPersona = (Rows.FindControl("Label5") as Label);
                    ...
                    if (AbrevBanco == "BCP")
                    {
                        ...
                        textfile += Convert.ToString(IdCont) + "," + Tipo + "," + Convert.ToString(NroCuenta) + "," + Reemplazar(MontoAdelanto) + "," + "," + Descripcion;
                    ...
                    else
                    {
                        ...
                        int CodCiudad = 701;
                        Tipo = "ACH";
                        textfile += Convert.ToString(IdCont) + "," + Tipo + "," + Convert.ToString(NroCuenta) + "," + Reemplazar(MontoAdelanto) + "," + "," + "," + AbrevBanco;
