...
{
    public partial class FormListaSolicitudPagosCarroGuia : System.Web.UI.Page
    {
        ...
        protected void DtgListaSolicitudPagoPagarCarroGuia_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                bit.Accion = "El Usuario va colocar Imagen de Carro Guia";

                int bi = NegBitacora.GuardarBitacora(bit);
                EntPermisosPagos Per = NegPermisosPagos.BuscarPermiso(us.Id_Usuario);
                if (Per.ImagenesPagadas == 1)
                {
                    string ConciliacionId = e.CommandArgument.ToString();
                    Response.Redirect("FormImagenesCarroGuia.aspx?Id=" + ConciliacionId);
                }
