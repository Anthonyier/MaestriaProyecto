...
{
    public partial class FormImagenesAnticipoChequeInter : System.Web.UI.Page
    {
            ...
            if (IdImgAnticipo == 0)
            {
                ...
                if (this.FileUpAnticip.HasFile)
                {
                    ...
                    if (DocAnticip.Imagen != null && MaestProv != 0)
                    {
                        ...
                        string NroBanco = TextNroBancarioAnticip.Text;
                        double MontoDolar = Math.Round(MontoAnticipo / 6.96, 2);
                        int modPago = 103;
                        string modalidad = "TRANSFERENCIA ELECTRONICA";
                        int IdCuenta = Convert.ToInt32(DdlCuentaBancaria.Text); 
                        bool Ver = NegImagenesAnticipos.MaestroProveedorCheqInter(DocAnticip.FechaAnticipo, IdPer, IdUsuario, MontoDolar, DocAnticip, idTipoPago, IdPasivo, NroBanco, modPago, modalidad,IdCuenta);
                        if (Ver == true)
                        {
