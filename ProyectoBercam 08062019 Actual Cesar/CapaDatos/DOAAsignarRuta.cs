...
namespace CapaDatos
{
    public class DOAAsignarRuta
    {
        ...
        public static int ObtenerPeriodo(int dia,int mes)
        {
            if (mes == 1 && dia <= 15)
            {
                return 2;
        ...
        public static void GuardarAnticipo(EntDetalle_Recepcion Detrec, int Recep)
        {
            EntAnticipo Ticipo = new EntAnticipo();
            SqlCommand cmd = null;
            SqlTransaction myTrans = null;
            ClaseConexion Conexion = new ClaseConexion();
            SqlConnection cnx = Conexion.conectar();
            Ticipo.Monto = Detrec.Monto_Anticipo;
            Ticipo.fecha = Detrec.FechaCarga;
            Ticipo.IdCamion = EncontrarIdCamion(Detrec.Placa_Camion);
            Ticipo.Recp = Recep ;
            Ticipo.Derecp = Detrec.Id_Detalle;
            Ticipo.Estado = 0;
            try
            {
                cnx.Open();
                ...
                string sql = "Insert into Anticipos (Fecha,Monto,IdCamion,IdRecepcion,IdDetalleRecepcion,Estado) values" +
                    "(@Fecha,@Monto,@IdCamion,@IdRecepcion,@IdDetalleRecepcion,@Estado)";
                cmd = new SqlCommand(sql, cnx);
                cmd.Parameters.AddWithValue("@Fecha", Ticipo.fecha);
                cmd.Parameters.AddWithValue("@Monto", Ticipo.Monto);
                cmd.Parameters.AddWithValue("@IdCamion", Ticipo.IdCamion);
                cmd.Parameters.AddWithValue("@IdRecepcion", Ticipo.Recp);
                cmd.Parameters.AddWithValue("@IdDetalleRecepcion", Ticipo.Derecp);
                cmd.Parameters.AddWithValue("@Estado", Ticipo.Estado);
                cmd.Transaction = myTrans;
                cmd.ExecuteNonQuery();
        ...
        public static SqlDataReader BuscarMesAño(DateTime Fecha)
        {
            SqlCommand cmd = null;
            SqlDataReader dr = null;
            try
            {
                ClaseConexion cn = new ClaseConexion();
                SqlConnection cnx = cn.conectar();
                cmd = new SqlCommand("BuscarMesAño", cnx);
                cmd.Parameters.AddWithValue("@FechaCarga", Fecha);
                cmd.CommandType = CommandType.StoredProcedure;
                cnx.Open();
                dr = cmd.ExecuteReader();
